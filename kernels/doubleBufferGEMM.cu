...
#pragma unroll
        ...
        // Compute C_reg while the next tile is staged in registers.
#pragma unroll
...
#pragma unroll
    ...
    const int last_pointer = (N_ - 1) % 2;
#pragma unroll
        for (int tn = 0; tn < sn; tn++)
        {
            // Load one A column and one B row from shared memory into registers.
            // A was transposed when stored, so both loads are row-wise here.
            FLOAT4(A_reg_compute[0]) = FLOAT4(A_smem[last_pointer][tn][T_y * ty / 2]);
            FLOAT4(A_reg_compute[4]) = FLOAT4(A_smem[last_pointer][tn][T_y * ty / 2 + sm / 2]);
            FLOAT4(B_reg_compute[0]) = FLOAT4(B_smem[last_pointer][tn][T_x * tx / 2]);
            FLOAT4(B_reg_compute[4]) = FLOAT4(B_smem[last_pointer][tn][T_x * tx / 2 + sm / 2]);
