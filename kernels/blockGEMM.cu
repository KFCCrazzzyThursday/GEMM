    ...
    const int M, const int N, const int K)
{
    ...
    // One block loads an A tile of size sm*sn using BLOCK_SIZE^2 threads.
    // Each thread loads sn*ty/BLOCK_SIZE elements.
    ...
    for (int n = 0; n < N_; ++n)
    {
        ...
        // Compute indices that depend on n.
...
#pragma unroll
                ...
                {
                    // Each step uses one column of A and one row of B.
    ...
    // Store C_reg back to global memory.
